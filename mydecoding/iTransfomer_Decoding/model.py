...
class ChunkKLDivergenceBatchMean(torch.autograd.Function):
    ...
    def _chunked_logsumexp(logits: torch.Tensor, chunk_size: int) -> torch.Tensor:
        ...
        for start in range(0, vocab_size, chunk_size):
            ...
            chunk_lse = torch.logsumexp(logits[:, start:end].float(), dim=-1)
            running_lse = chunk_lse if running_lse is None else torch.logaddexp(running_lse, chunk_lse)
        if running_lse is None:
            return torch.zeros((n_rows,), device=logits.device, dtype=torch.float32)
        return running_lse
    ...
    def forward(ctx, draft_logits, teacher_logits, chunk_size: int = 8192):
        ...
        chunk_size = max(1, min(int(chunk_size), vocab_size))
        ctx.chunk_size = chunk_size
        ctx.n_rows = n_rows

        lse_p = ChunkKLDivergenceBatchMean._chunked_logsumexp(draft_logits, chunk_size)
        lse_q = ChunkKLDivergenceBatchMean._chunked_logsumexp(teacher_logits, chunk_size)
