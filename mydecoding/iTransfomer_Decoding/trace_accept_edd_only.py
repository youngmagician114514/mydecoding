...
def edd_greedy_draft_tokens(
    ...
    encoder_out: torch.Tensor,  # (1,T,H) or (1,K,T,H)
    start_token: torch.Tensor,  # (1,1)
    steps: int,
    eos_id: int,
) -> torch.Tensor:
    encoder_self, memory_base = edd._split_encoder_out(encoder_out)
...
def run_trace(
    ...
    while cur_gen < max_new_tokens:
        ...
        encoder_out = _append_encoder_out(encoder_out, outv, hidden_layer, layer_indices, acc_ids)
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.model_checkpoint, use_fast=True, padding_side="left")
